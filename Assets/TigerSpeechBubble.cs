...
public class TigerSpeechBubble : MonoBehaviour
{
    ...
    // Bubble — now its own Local2D layer (#439/#491), composited "glass over 3D"
    // in the 2D zone, instead of a surround child. The surround canvas stays for
    // the editor gizmos + the 2D-zone backdrop; the 3D zone keeps its canvas
    // sub-rect (Kooima) via PushCanvasRect.
    ...
    private RectInt m_BubbleBoxPx = new RectInt(0, 0, 0, 0); // bubble rect (window px) for the click catch-region
    ...
    void OnDisable()
    {
        ...
        m_CanvasRT = null; m_BubbleRT = null; m_TailRT = null; m_BubbleL2D = null;
    ...
    void BuildUI()
    {
        ...
        // Gizmos stay on the surround canvas (full-window editor overlay).
        BuildGizmos(canvasGO.transform);

        // The speech bubble is its own Local2D layer (#439/#491) — composited
        // "glass over 3D" at the 2D-zone rect, driven by the region editor. This
        // is the VK-avatar design and fixes the surround path's disappearing bug
        // (Local2D recreates its overlay swapchain when the RT changes).
        ...
        m_BubbleL2D.useExplicitRect = true; // the editor supplies the exact panel-px rect
    ...
    void BuildBubble(Transform parent)
    {
        ...
        // The Local2D layer stretches the rendered RT into the dest rect, so the
        // bubble tracks the editor's 2D-zone rect.
    ...
    void ApplyLayout()
    {
        ...
        // --- Bubble = a Local2D layer filling the 2D zone (inset by a margin).
        // The panel/tail fill the layer's RT (set in BuildBubble); here we just
        // place the layer at the 2D-zone rect in client-window pixels. The runtime
        // composites it "glass over 3D" with the implicit mask.
        float m = kBubbleMargin * vw;
        float bx = zx + m;
        float by = vy + m;
        float bw = Mathf.Max(40f, zw - 2f * m);
        float bh = Mathf.Max(40f, zh - 2f * m);
        if (m_BubbleL2D != null)
        {
            m_BubbleL2D.SetExplicitRect(Mathf.RoundToInt(bx), Mathf.RoundToInt(by),
                                        Mathf.RoundToInt(bw), Mathf.RoundToInt(bh));
        }
        m_BubbleBoxPx = new RectInt(Mathf.RoundToInt(bx), Mathf.RoundToInt(by),
                                    Mathf.RoundToInt(bw), Mathf.RoundToInt(bh));
        // Padding is in RT pixels (the panel renders into a fixed-resolution RT).
    ...
    private void PushBubbleMask()
    {
        ...
        // The bubble now lives on its own Local2D layer, so we no longer have it
        // on the surround canvas to silhouette-map. Use its known window rect
        // (computed in ApplyLayout) as a solid box catch-region — good enough for
        // click routing; the Local2D layer owns the visual.
        var box = m_BubbleBoxPx;
        if (box.width <= 0 || box.height <= 0) { ClearMask(); return; }
        if (m_MaskPushed && box.Equals(m_LastMaskBox)) return;
        var mask = new byte[] { 255, 255, 255, 255 }; // 2x2 solid → whole-rect
        var handle = GCHandle.Alloc(mask, GCHandleType.Pinned);
